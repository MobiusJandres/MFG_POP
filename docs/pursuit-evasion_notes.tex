...
\begin{abstract}
This document establishes a formal mathematical framework for modeling the interaction between a continuum of minor players (a crowd) and an active, strategic major player (an evading goal) within a bounded spatial domain containing obstacles. The system is formulated as a Major-Minor Pursuit-Evasion Mean Field Game (PE-MFG). We detail the backward Hamilton-Jacobi-Bellman (HJB) equation governing optimal drone navigation, the forward Kolmogorov-Fokker-Planck (KFP) equation modeling crowd mass transport, and the state-costate differential equations determining the evader's optimal escape trajectory. Finally, an extended damped Picard fixed-point scheme is presented for numerical computation.
\end{abstract}
...
Each individual drone minimizes a cost functional penalizing running distance to the dynamic goal $y(t)$, spatial congestion, and control effort.
